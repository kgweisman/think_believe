...
d3_long %>%
  ...
  filter(country != "Ghana (undergrads)") %>%
  ...
  theme(#axis.text.x = element_text(angle = 45, hjust = 1, vjust = 1),
        legend.position = "top") +
  labs(x = "Vignette type", y = "Percent of responses", fill = "Response")
ggsave("../figures/cogsci2020/fig03.png")
```
...
Replotted to highlight pairings of questions
...
d3_long %>%
  ...
  ggplot(aes(x = category, 
             ...
             fill = factor(response_cat,
                           levels = c(NA, "think", "believe"), 
                           exclude = NULL))) +
  facet_grid(country_n ~ reorder(str_wrap(question_text_short, 30), order), 
...
```{r}
regtab_fun(r3.1, std_beta = T) %>% regtab_style_fun(row_emph = c(2, 8:12))
```
...
- Country (Gh.)2: Participants in the second sample from Ghana were generally less likely than other participants to say "believe," collapsing across question categories.
...
- **Category (religious) x Country (Gh.)2**: The difference in rates of "believe" responses between question categories was no smaller or larger in the second sample from Ghana than in other countries.
...
```{r, include = F}
regtab_fun(r3.1a, std_beta = T) %>% regtab_style_fun(row_emph = c(2, 7:10))
```
...
```{r}
bind_rows(regtab_fun(r3.1_us, std_beta = T) %>% mutate(Country = "US"),
          regtab_fun(r3.1_gh, std_beta = T) %>% mutate(Country = "Ghana"),
          regtab_fun(r3.1_gh2, std_beta = T) %>% mutate(Country = "Ghana (undergrads)"),
          regtab_fun(r3.1_th, std_beta = T) %>% mutate(Country = "Thailand"),
          regtab_fun(r3.1_ch, std_beta = T) %>% mutate(Country = "China"),
          regtab_fun(r3.1_vt, std_beta = T) %>% mutate(Country = "Vanuatu")) %>%
...
```{r}
bind_rows(regtab_fun(r3.6_eng, std_beta = T) %>% 
            mutate(Language = "English"),
          regtab_fun(r3.6_noeng, std_beta = T) %>% 
...
```{r}
bind_rows(regtab_fun(r3.8_eng_gh, std_beta = T) %>% 
            mutate(Country = "Ghana", Language = "English"),
          regtab_fun(r3.8_noeng_gh, std_beta = T) %>% 
            mutate(Country = "Ghana", Language = "No English"),
          regtab_fun(r3.8_eng_th, std_beta = T) %>% 
            mutate(Country = "Thailand", Language = "English"),
          regtab_fun(r3.8_noeng_th, std_beta = T) %>% 
            mutate(Country = "Thailand", Language = "No English"),
          regtab_fun(r3.8_eng_vt, std_beta = T) %>% 
            mutate(Country = "Vanuatu", Language = "English"),
          regtab_fun(r3.8_noeng_vt, std_beta = T) %>% 
...
```{r}
bind_rows(regtab_fun(r3.9_eng, std_beta = T) %>% 
            mutate(Language = "English"),
          regtab_fun(r3.9_noeng, std_beta = T) %>% 
            mutate(Language = "No English")) %>%
  select(Language, everything()) %>%
